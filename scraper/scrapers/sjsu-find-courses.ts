        ...
        if (table.length) {
            ...
            table.find("tbody tr").each((_, element) => {
                ...
                if (cells.length >= 10) {
                    ...
                    const professors = [...new Set(rawProfessor
                        .split(" / ")
                        //sometimes pronouns are listed so remove them with replace. Replace accented letters with normalize then replace
                        .map(name => name.replace(/\s*\([^)]*\)/g, "").normalize("NFKD").replace(/[\u0300-\u036f]/g, "").trim()))];
            
                    professors.forEach(professor => {
                        const course = { semester, title, subject, courseNumber, professor };
